...
namespace PalworldSaveDecoding
{
    public class WorkAssign
    {
        ...
        public static WorkAssign Read(GvasFileReader reader, MessageCollection? messages = null)
        {
            var localMessages = new MessageCollection();
            var result = new WorkAssign();
            ...
            while (structName != "None") {
                ...
                switch (structName) {
                    ...
                    default:
                        if (messages == null)
                            throw new InvalidDataException($"Unknown WorkAssign struct {structName}");
                        localMessages.Add(new Message("StructName", "WorkAssign", $"Unknown structName {structName} of type {typeName}", null));
                        reader.Skip(size);
                        break;
                }
        ...
        private void DecodeRawData(byte[] data)
        {
            ...
            using (var reader = new GvasFileReader(new MemoryStream(data), true)) {
                ...
                IsFixed = reader.ReadUInt32() > 0;

                if (!reader.IsBaseStreamEnds)
                    throw new InvalidDataException("WorkAssign raw data invalid length");
            }
        }
    }
}
